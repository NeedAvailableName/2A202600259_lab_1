...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        # Trim history to the last 3 turns (each turn has user and assistant message)
        # So last 6 messages if we count both. The hint says "last 3 conversation turns".
        # Usually a turn is (user, assistant). So 3 turns = 6 messages.
        if len(history) > 6:
            history = history[-6:]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    """
    header = f"{'Prompt':<40} | {'GPT-4o Response':<40} | {'Mini Response':<40} | {'4o Latency':<10} | {'Mini Latency':<10}\n"
    separator = "-" * len(header) + "\n"
    table = header + separator
    
    for res in results:
        prompt = (res["prompt"][:37] + "...") if len(res["prompt"]) > 40 else res["prompt"]
        gpt4o = (res["gpt4o_response"].replace('\n', ' ')[:37] + "...") if len(res["gpt4o_response"]) > 40 else res["gpt4o_response"].replace('\n', ' ')
        mini = (res["mini_response"].replace('\n', ' ')[:37] + "...") if len(res["mini_response"]) > 40 else res["mini_response"].replace('\n', ' ')
        latency4o = f"{res['gpt4o_latency']:.2f}s"
        latency_mini = f"{res['mini_latency']:.2f}s"
        
        table += f"{prompt:<40} | {gpt4o:<40} | {mini:<40} | {latency4o:<10} | {latency_mini:<10}\n"
